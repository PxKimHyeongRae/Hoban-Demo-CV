#!/usr/bin/env python3
"""v3 모델 SAHI 1280x720 재평가 (best.pt, image_size=1280)"""
import os, time
...
MODEL = "/home/lay/hoban/hoban_go2k_v3/weights/best.pt"
...
for i, f in enumerate(images):
    ...
    preds_tile[f] = [(p.category.id, p.score.value,
...
print("-" * 60)
for c in CONFS:
    n, tp, fp, fn, p, r, f1 = evaluate(all_gt, preds_tile, c)
    print(f"{c:>5.2f} {n:>6} {tp:>6} {fp:>6} {fn:>6} {p:>7.3f} {r:>7.3f} {f1:>7.3f}")

# 2) SAHI + perform_standard_pred
print(f"\n{'='*80}")
print("v3 SAHI 1280x720 + perform_standard_pred")
print("=" * 80)
t0 = time.time()
preds_std = {}
for i, f in enumerate(images):
...
print("비교 기준 (v2)")
print("=" * 80)
print("v2 타일만     conf=0.50: F1=0.912, P=0.884, R=0.942, FP=208")
