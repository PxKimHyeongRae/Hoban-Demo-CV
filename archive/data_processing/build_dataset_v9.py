"""
datasets_v9 빌드
- helmet_o/x: 사용자 제공 (area >= 2%, YOLO format, helmet_v9/ 폴더)
- person: coco_person_filtered (area >= 2%)
- fallen: fallen_pool_filtered (0.5% <= area <= 70%)
- negative: 빈 라벨 이미지 1K (negative_samples/ 폴더)
- 클래스당 6,000 bbox 균형
"""
import shutil
import random
...
# 소스 경로
HELMET_DIR = Path(r"D:\task\hoban\dataset\helmet_v9")       # 사용자 제공: images/, labels/ (cls 0, 1)
PERSON_DIR = Path(r"D:\task\hoban\dataset\coco_person_filtered")  # 필터링 완료
FALLEN_DIR = Path(r"D:\task\hoban\dataset\fallen_pool_filtered")  # 필터링 완료
NEG_DIR = Path(r"D:\task\hoban\dataset\negative_samples")    # 사용자 제공: images/
...
def main():
    ...
    print("=== datasets_v9 빌드 ===\n")

    # === 1. Helmet (cls 0, 1) ===
    print("[1] Helmet 데이터 (사용자 제공)")
    ...
    if NEG_DIR.exists() and (NEG_DIR / "images").exists():
        ...
        for i, img in enumerate(neg_imgs):
            if not img.suffix.lower() in [".jpg", ".jpeg", ".png"]:
                continue
