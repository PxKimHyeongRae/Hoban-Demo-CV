#!/usr/bin/env python3
"""v26 3-class: L티어 헬멧 + fallen 대량 보강 (unified_safety + v24)

v25 대비 변경:
  - fallen 데이터 대폭 증가: unified_safety_all(area<5%) + v24(area<10%)
  - copy_paste 0.15 → 0.4 (fallen을 CCTV 배경에 합성 강화)
  - scale 0.5 → 0.7 (소형 객체 학습 강화)
...
사용법:
  python train_go3k_v26.py --prepare   # 데이터셋 빌드
  python train_go3k_v26.py --train     # 학습
  python train_go3k_v26.py --resume    # 이어서 학습
"""
import os
import shutil
import random
...
OUT_DIR = f"{HOBAN}/datasets_go3k_v26"
...
def extract_v24_fallen(max_area=0.10):
    ...
    for lbl_name in os.listdir(V24_TRAIN_LBL):
        ...
        if not fallen_bboxes:
            continue
        # 최소 하나의 fallen bbox가 area < max_area
        ...
        if valid_bboxes:
            ...
            if os.path.exists(os.path.join(V24_TRAIN_IMG, img_name)):
                ...
                fallen.append({
                    ...
                    'source': 'v24',
                    'bboxes': bboxes,  # 원본 전체 (이미 class 2)
                })
    return fallen


def extract_unified_fallen(max_area=0.05, max_images=5000):
...
def prepare():
    """v26 데이터셋 빌드"""
    print("=" * 60)
    print("  v26 3-class 데이터셋 빌드")
    print("  L 티어 helmet 2,000장 + fallen 대량 보강")
    print("=" * 60)
    ...
    # ── 3. unified_safety_all fallen (area < 5%, class 4→2) ──
    print("\n[3/5] unified_safety_all fallen (area < 5%)...")
    # v24와 겹치지 않는 이미지만 (prefix로 구분 가능)
    # v24 + unified 합쳐서 최대 1,000장으로 제한
    remaining = max(0, 1000 - v24_count)
    unified_fallen = extract_unified_fallen(max_area=0.05, max_images=remaining)
    unified_count = 0
    for item in unified_fallen:
        # unified 이미지에 'uf_' prefix 추가하여 이름 충돌 방지
...
def train(batch=6, epochs=100, resume=False):
    """v26 학습"""
    ...
    model.train(
        ...
        mosaic=1.0,
        mixup=0.1,
        copy_paste=0.4,      # v25 0.15 → 0.4 (fallen 합성 강화)
        hsv_h=0.015,
        hsv_s=0.7,
        hsv_v=0.4,
        scale=0.7,            # v25 0.5 → 0.7 (소형 객체 학습 강화)
        translate=0.1,
        degrees=5.0,
        fliplr=0.5,
        erasing=0.15,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="v26 3-class (fallen 대량 보강)")
