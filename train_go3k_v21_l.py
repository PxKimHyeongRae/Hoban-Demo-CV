#!/usr/bin/env python3
"""
go3k v21-l: yolo26l COCO pretrained + v19 데이터셋

모델: yolo26l.pt (26.3M params, COCO pretrained) — yolo26m 대비 1.2x
데이터: v19 dataset (v16 base 10,564 + helmet_off 946 + neg 960 = 12,470 train)
학습: 100ep, SGD lr0=0.005, patience=20

사용법:
  python train_go3k_v21_l.py              # 학습 시작
  python train_go3k_v21_l.py --resume     # 이어서 학습
  python train_go3k_v21_l.py --batch 5    # 배치 조절 (기본 4)
"""
import argparse
from ultralytics import YOLO

HOBAN = "/home/lay/hoban"
MODEL = f"{HOBAN}/yolo26l.pt"
DATA = f"{HOBAN}/datasets_go3k_v19/data.yaml"
PROJECT = HOBAN
NAME = "hoban_go3k_v21_l"
...
def main():
    ...
    print(f"=== go3k v21-l: yolo26l COCO pt ===")
    print(f"  Model: {MODEL} (26.3M params)")
    print(f"  Data:  {DATA}")
