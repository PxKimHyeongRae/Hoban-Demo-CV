#!/usr/bin/env python3
"""
go3k v20: v17 base + CVAT 검수 helmet 946장 + CVAT 검수 negative 960장
...
사용법:
  1. 데이터셋 준비: python train_go3k_v20.py --prepare
  2. 학습 시작:     python train_go3k_v20.py
  3. 이어서 학습:    python train_go3k_v20.py --resume
"""
import argparse
import os
import shutil
...
V20_DATASET = f"{HOBAN}/datasets_go3k_v20"
...
def prepare_dataset():
    """v20 데이터셋 준비: v16 base + helmet_off + negative"""
    print("=" * 60)
    print("  v20 데이터셋 준비")
    print("=" * 60)

    train_img = f"{V20_DATASET}/train/images"
    train_lbl = f"{V20_DATASET}/train/labels"
    val_img = f"{V20_DATASET}/valid/images"
    val_lbl = f"{V20_DATASET}/valid/labels"
    ...
    for src in v16_train_imgs + v16_train_lbls:
        dst = src.replace(V16_DATASET, V20_DATASET)
    ...
    # 3. data.yaml
    yaml_path = f"{V20_DATASET}/data.yaml"
    with open(yaml_path, "w") as f:
        f.write(f"path: {V20_DATASET}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Train go3k v20")
    ...
    project = HOBAN
    name = "hoban_go3k_v20"
    data_yaml = f"{V20_DATASET}/data.yaml"

    if not os.path.exists(data_yaml):
        print("데이터셋 없음. 먼저 --prepare 실행:")
        print("  python train_go3k_v20.py --prepare")
        return
    ...
    print(f"=== go3k v20: v17 + CVAT helmet 946 + CVAT neg 960 ===")
