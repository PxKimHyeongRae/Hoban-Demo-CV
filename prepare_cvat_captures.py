#!/usr/bin/env python3
"""
captures_labels_3k → CVAT 3개 task로 패키징
YOLO 1.1 format (obj.data + obj.names + train.txt + labels)

실행: python prepare_cvat_captures.py
"""
import os
import zipfile

LABEL_DIR = "/home/lay/hoban/datasets/captures_labels_3k"
CAP_DIR = "/home/lay/video_indoor/static/captures"
OUT_BASE = "/home/lay/hoban/datasets/cvat_captures"
N_TASKS = 3
CLASS_NAMES = ["person_with_helmet", "person_without_helmet"]

os.makedirs(OUT_BASE, exist_ok=True)

# 라벨 → 이미지 매칭 (카메라 디렉토리 자동 탐색)
label_files = sorted(os.listdir(LABEL_DIR))
pairs = []
for lf in label_files:
    img_name = lf.replace(".txt", ".jpg")
    cam = img_name.split("_")[0]
    img_path = os.path.join(CAP_DIR, cam, img_name)
    if os.path.exists(img_path):
        pairs.append((lf, cam, img_name, img_path))

print(f"라벨: {len(label_files)}개, 매칭: {len(pairs)}개")

# 3개 task 분할
batch_size = (len(pairs) + N_TASKS - 1) // N_TASKS

for task_idx in range(N_TASKS):
    start = task_idx * batch_size
    end = min(start + batch_size, len(pairs))
    batch = pairs[start:end]
    part = task_idx + 1

    print(f"\n[Part {part}/{N_TASKS}] {len(batch)}장")

    # annotations.zip
    ann_path = os.path.join(OUT_BASE, f"captures_part{part}_annotations.zip")
    with zipfile.ZipFile(ann_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("obj.data", f"classes = {len(CLASS_NAMES)}\ntrain = data/train.txt\nnames = data/obj.names\nbackup = backup/\n")
        zf.writestr("obj.names", "\n".join(CLASS_NAMES) + "\n")

        train_lines = []
        for lf, cam, img_name, img_path in batch:
            train_lines.append(f"data/obj_train_data/{img_name}")
            zf.write(os.path.join(LABEL_DIR, lf), f"obj_train_data/{lf}")
        zf.writestr("train.txt", "\n".join(train_lines) + "\n")

    # images.zip
    img_zip_path = os.path.join(OUT_BASE, f"captures_part{part}_images.zip")
    with zipfile.ZipFile(img_zip_path, 'w', zipfile.ZIP_STORED) as zf:
        for i, (_, cam, img_name, img_path) in enumerate(batch):
            if i % 200 == 0:
                print(f"  이미지: {i}/{len(batch)}...")
            zf.write(img_path, img_name)

    ann_size = os.path.getsize(ann_path) / 1024 / 1024
    img_size = os.path.getsize(img_zip_path) / 1024 / 1024
    print(f"  → annotations: {ann_size:.1f}MB, images: {img_size:.1f}MB")

total_size = sum(
    os.path.getsize(os.path.join(OUT_BASE, f))
    for f in os.listdir(OUT_BASE)
) / 1024 / 1024 / 1024

print(f"\n{'=' * 60}")
print(f"완료! {len(pairs)}장 → {N_TASKS}개 task")
print(f"총 용량: {total_size:.2f}GB")
print(f"출력: {OUT_BASE}")
